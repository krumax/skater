...
import { useDefeatData } from '../../hooks/useDefeatData';
...
export default function DefeatMatrix({ rounds, player }) {
  const { map } = useDefeatData(rounds, player);
              ...
              <tr key={row.type}
                ...
                {COL_SPECS.map(col => (
                  <td key={col.id} style={{ padding: '0.25rem', textAlign: 'center', backgroundColor: col.isSpecial ? 'rgba(116,91,0,0.04)' : 'transparent' }}>
                    <MatrixCell val={map[row.type]?.[col.id]} isSpecial={col.isSpecial} isDefense />
                  </td>
                ))}
              </tr>
            ))}
                  ...
                  {COL_SPECS.map((col, idx) => {
                    ...
                    return (
                      <td key={col.id} style={{ padding: '0.25rem', textAlign: 'center', backgroundColor: 'rgba(116,91,0,0.05)' }}>
                        <MatrixCell val={val} isSpecial={true} isDefense />
                      </td>
                    );
                  })}
                </tr>
